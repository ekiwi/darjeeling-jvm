...
public class Node
{
	...
	// tracks how many tries were needed to send a unicast message for the last HISTORY_LENGTH messages 
	...
	public short getCost()
	{
		...
		{
			for (byte i=0; i<historySize; i++) cost += triesHistory[(historyStart+HISTORY_LENGTH-i-1)%HISTORY_LENGTH];
	...
	public void updateHistory(byte tries)
	{
		...
		if (historySize<HISTORY_LENGTH) historySize++;
	}
	
	/**
	 * @param sendErrors the number of send errors (undelivered packets) to this node
	 */
