...
public class DataEngine extends AbstractPacketListener
{
	...
	public final static int DATA_SEND_INTERVAL = 5000;
		...
		public boolean isDuplicate(short id, short thl, short seq)
		{
			synchronized(this)
			{
				...
				for (short i=0; i<size; i++)
				{
					...
					{
						// duplicate found, refresh the item by moving it to the head of the queue
						for (short j=i; j>=0; j--)
		...
		new Thread() {
			public void run()
			{
				while (true)
				{
					...
					{
						Debug.print("SendDataLoop died with throwable: ");
	...
	private void sendDataLoop()
	{
		...
		while (true)
		{
			...
			// send data packet if there is a route to the sink, and if the routing engine is not a sink
			if (routingEngine.getParent()!=-1 && !routingEngine.isSink() && provider!=null)
			{
				...
				try {
                    ...
                    Debug.print("DataEndgine: sending CtpDataFrame: ");
	...
	public void packetReceived(Packet packet)
	{
		if (packet instanceof CtpDataFrame)
		{
            ...
            Debug.print("DataEndgine: CtpDataFrame received: ");
			...
			// deliver or forward the message based on whether this node is a sink or not
			if (routingEngine.isSink())
			{
				if (consumer!=null)
				{
					short data[] = new short[dataFrame.getDataLength()];
					for (byte i=0; i<data.length; i++) data[i] = dataFrame.getDataItem(i);
